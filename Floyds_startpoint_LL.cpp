...
void floyds_startpoint(Node*head){
    ...
    if(fast==nullptr){cout<<"not any loop";return;}
    slow=head;
    while(slow!=fast){
        slow=slow->next;
        fast=fast->next;
    }cout<<"the loop starts at:- "<<slow->data<<endl;
...
int main(){
Node*first=new Node(1);first->next=first;
